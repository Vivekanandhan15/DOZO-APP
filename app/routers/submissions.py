...
def submit(data: SubmissionCreate, user=Depends(require_role(["STUDENT"])),
    ...
    return submit_assignment(db, student.student_id, data)

# Teacher grading submission
@router.put("/{submission_id}", dependencies=[Depends(require_role(["ADMIN", "TEACHER"]))])
...
from app.services.submissions import get_all_submissions, get_student_submissions, get_teacher_submissions
...
@router.get("/all", response_model=List[SubmissionOut], dependencies=[Depends(require_role(["ADMIN", "TEACHER"]))])
def all_submissions(user=Depends(require_role(["ADMIN", "TEACHER"])), db: Session = Depends(get_db)):
    # Both Admin and Teacher should only see submissions for tasks they created
    return get_teacher_submissions(db, user.user_id)
